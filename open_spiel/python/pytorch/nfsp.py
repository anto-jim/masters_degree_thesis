...
class NFSP(rl_agent.AbstractAgent):
    ...
    if self._mode == MODE.BEST_RESPONSE:
      ...
      if not time_step.last():
        ...
        action_values, action, probs = self._act(
            np.asarray(info_state), legal_actions_mask
        )
  ...
  def add_transition(
      ...
      agent_output: rl_agent.StepOutput,
  ) -> None:
    ...
    """
    legal_actions = np.asarray(
        time_step.observations["legal_actions"][self.player_id], dtype=np.int32
    )
    legal_actions_mask = np.zeros(self._num_actions, dtype=bool)
    legal_actions_mask[legal_actions] = True
  ...
  def _learn(self) -> None | float:
    ...
    action_probs = torch.tensor(
        transitions.action_probs, device=self._device, dtype=torch.float32
    ).detach()
  ...
  def save(
      self, checkpoint_dir: pathlib.Path, save_optimiser: bool = True
  ) -> None:
